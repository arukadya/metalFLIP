...
, _animationIndex(0)
{
    ...
    buildBuffers();
    simulator.setParamators(dx, dt, rho, radius, thr, gamma);
...
void Renderer::draw( MTK::View* pView )
{
    ...
    pCmd->addCompletedHandler( ^void( MTL::CommandBuffer* pCmd ){//commandBuffer終了時に呼び出される関数を指定
        dispatch_semaphore_signal( pRenderer->_semaphore );//同期処理の終了宣言
    });
    //simulator.onecycle();
    _angle += 0.002f;
